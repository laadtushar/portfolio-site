...
export function SkillsCup() {
  ...
  // Position to the right of the main coffee cup
  let position = breakpoints.menu ? [6.5, -0.8, 3.5] : [2.2, -2.9, 3.8];
  let rotation = [0, 0, 0];

  if (scene === 'about') {
    position = [4.5, -6.5, 3.02];
    rotation = [0.00, 0.00, 2 + Math.PI * 2];

    if (breakpoints.projects) {
      position = [7, -10.5, 3.02];
  ...
  return (
    ...
    >
      ...
      <Text
        ...
        visible={cupButtonVisible1}
      >
        {scene === 'about' ? 'Back'.toUpperCase() : 'Skills'.toUpperCase()}
      </Text>
      ...
      <ThreeButton
        ...
        width={2}
        height={2}
        description={'A coffee mug with the word "Skills" on it.'}
        activationMsg="Mug tips, coffee spills everywhere, revealing skills in an artistic window."
        ...
        onClick={() => {
          ...
          if (typeof window !== 'undefined') {
            window.location.href = '/?slide=skills';
          }
        }}
      />
      )}
